...
double predecir(double **datos, int filas, int columnas, int k, int h, int filaActual)
{
    ...
    {
        distancias[i].distancia = calcularDistancia(datos[i], datos[filaActual], h);
        distancias[i].indice = i;
    }

    // Ordenamos las distancias de menor a mayor
    qsort(distancias, filas, sizeof(DistanciaIndice), compararDistancias); 
...
int main(int argc, char **argv)
{
    ...
    int filas, columnas;
    double **datos = NULL;
    double *datosContiguos = NULL; // Nuevo array para datos contiguos
    int filasPorProceso;
    int filasRestantes;
    ...
    if (rank == 0) {
        ...
        // Enviamos 'filas' y 'columnas' a todos los procesos
        ...
        // Crear una copia contigua de la matriz 'datos'
        datosContiguos = (double *)malloc(filas * columnas * sizeof(double));
        if (datosContiguos == NULL) {
            perror("Error al asignar memoria para datosContiguos");
            MPI_Abort(MPI_COMM_WORLD, 1);
        }
        for (int i = 0; i < filas; ++i) {
            memcpy(&datosContiguos[i * columnas], datos[i], columnas * sizeof(double));
        }

        fclose(archivo);
    } else {
        // Los demás procesos reciben 'filas' y 'columnas'
        MPI_Bcast(&filas, 1, MPI_INT, 0, MPI_COMM_WORLD); 
        MPI_Bcast(&columnas, 1, MPI_INT, 0, MPI_COMM_WORLD);
    }
    ...
    // ¡CORRECCIÓN!: Usar MPI_Scatter con 'datosContiguos'
    for (int i = 0; i < filasPorProceso; ++i) {
        MPI_Scatter(&datosContiguos[i * columnas], columnas, MPI_DOUBLE, 
                    datosLocal[i], columnas, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    }
    ...
    if (rank == 0)
    {
        ...
        imprimirMAPE(mapes, numProcesos); // ¡CORRECCIÓN!: Usar 'numProcesos' en lugar de 'filas'
    ...
    if (rank == 0)
    {
        ...
        free(datos);
        free(datosContiguos); // Liberar memoria de datosContiguos
